...
#include <zqrdecode/common/GridSampler.h>
...
#include <iostream>
#include <zqrdecode/common/detector/MathUtils.h>
#include <zqrdecode/NotFoundException.h>

using std::vector;
using zqrdecode::aztec::Detector;
using zqrdecode::aztec::Point;
...
using zqrdecode::common::detector::MathUtils;
...
void Detector::correctParameterData(Ref<zqrdecode::BitArray> parameterData, bool compact) {
  int numCodewords;
...
std::vector<Ref<Point> > Detector::getBullEyeCornerPoints(Ref<zqrdecode::aztec::Point> pCenter) {
...
Ref<BitMatrix> Detector::sampleGrid(Ref<zqrdecode::BitMatrix> image,
                                    Ref<zqrdecode::ResultPoint> topLeft,
                                    Ref<zqrdecode::ResultPoint> bottomLeft,
                                    Ref<zqrdecode::ResultPoint> bottomRight,
                                    Ref<zqrdecode::ResultPoint> topRight) {
  int dimension;
  if (compact_) {
...
void Detector::getParameters(Ref<zqrdecode::BitArray> parameterData) {
  nbLayers_ = 0;
  nbDataBlocks_ = 0;
...
Ref<BitArray> Detector::sampleLine(Ref<zqrdecode::aztec::Point> p1, Ref<zqrdecode::aztec::Point> p2, int size) {
...
bool Detector::isWhiteOrBlackRectangle(Ref<zqrdecode::aztec::Point> p1,
                                       Ref<zqrdecode::aztec::Point> p2,
                                       Ref<zqrdecode::aztec::Point> p3,
                                       Ref<zqrdecode::aztec::Point> p4) {
  int corr = 3;
...
int Detector::getColor(Ref<zqrdecode::aztec::Point> p1, Ref<zqrdecode::aztec::Point> p2) {
  float d = distance(p1, p2);
...
Ref<Point> Detector::getFirstDifferent(Ref<zqrdecode::aztec::Point> init, bool color, int dx, int dy) {
  int x = init->getX() + dx;
  int y = init->getY() + dy;
...
float Detector::distance(Ref<zqrdecode::aztec::Point> a, Ref<zqrdecode::aztec::Point> b) {
