...
const char* Decoder::getCharacter(zqrdecode::aztec::Decoder::Table table, int code) {
  switch (table) {
  case UPPER:
...
Ref<DecoderResult> Decoder::decode(Ref<zqrdecode::aztec::AztecDetectorResult> detectorResult) {
...
Ref<String> Decoder::getEncodedData(Ref<zqrdecode::BitArray> correctedBits) {
...
Ref<BitArray> Decoder::correctBits(Ref<zqrdecode::BitArray> rawbits) {
  //return rawbits;
...
Ref<BitArray> Decoder::extractBits(Ref<zqrdecode::BitMatrix> matrix) {
...
Ref<BitMatrix> Decoder::removeDashedLines(Ref<zqrdecode::BitMatrix> matrix) {
...
int Decoder::readCode(Ref<zqrdecode::BitArray> rawbits, int startIndex, int length) {
  int res = 0;
