...
#include <zqrdecode/qrcode/detector/AlignmentPatternFinder.h>
...
using std::min;
using std::max;
using zqrdecode::qrcode::Detector;
using zqrdecode::Ref;
using zqrdecode::BitMatrix;
using zqrdecode::ResultPointCallback;
using zqrdecode::DetectorResult;
using zqrdecode::PerspectiveTransform;
using zqrdecode::qrcode::AlignmentPattern;
using zqrdecode::common::detector::MathUtils;

// VC++
using zqrdecode::DecodeHints;
using zqrdecode::qrcode::FinderPatternFinder;
using zqrdecode::qrcode::FinderPatternInfo;
...
Ref<DetectorResult> Detector::processFinderPatternInfo(Ref<FinderPatternInfo> info){
  ...
  if (moduleSize < 1.0f) {
    throw zqrdecode::ReaderException("bad module size");
  }
  ...
  if (provisionalVersion->getAlignmentPatternCenters().size() > 0) {
    ...
    for (int i = 4; i <= 16; i <<= 1) {
      try {
        ...
        break;
      } catch (zqrdecode::ReaderException const& re) {
        (void)re;
        // try next round
      }
    }
...
int Detector::computeDimension(Ref<ResultPoint> topLeft, Ref<ResultPoint> topRight, Ref<ResultPoint> bottomLeft,
  ...
  case 3:
    ...
    throw zqrdecode::ReaderException(s.str().c_str());
  }
  return dimension;
}
...
float Detector::calculateModuleSizeOneWay(Ref<ResultPoint> pattern, Ref<ResultPoint> otherPattern) {
  ...
  if (zqrdecode::isnan_z(moduleSizeEst1)) {
    return moduleSizeEst2;
  }
  if (zqrdecode::isnan_z(moduleSizeEst2)) {
...
Ref<AlignmentPattern> Detector::findAlignmentInRegion(float overallEstModuleSize, int estAlignmentX, int estAlignmentY,
  ...
  if (alignmentAreaRightX - alignmentAreaLeftX < overallEstModuleSize * 3) {
    throw zqrdecode::ReaderException("region too small to hold alignment pattern");
  }
