...
Ref<GenericGFPoly> GenericGFPoly::addOrSubtract(Ref<zqrdecode::GenericGFPoly> other) {
...
Ref<GenericGFPoly> GenericGFPoly::multiply(Ref<zqrdecode::GenericGFPoly> other) {
  if (!(field_.object_ == other->field_.object_)) {
    throw IllegalArgumentException("GenericGFPolys do not have same GenericGF field");
  }
